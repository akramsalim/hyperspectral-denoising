...
from src.models import DownstreamModel, load_vit_weights  # Changed import
...
class SpectralEarthDataset(Dataset):
    ...
    def __init__(self, base_path, noise_std=0.27, stripe_intensity=0.5, mode='test'):
        ...
        # Get all .tif files recursively from enmap directory
        self.file_paths = []
...
def evaluate_on_new_dataset(model_path, config, save_dir):
    """
    Evaluate the regular model on the EnMAP dataset.
    ...
    with open(save_dir / 'test_results.txt', 'w') as f:
        f.write("=== Regular Model Test Results on EnMAP Dataset ===\n\n")
...
if __name__ == "__main__":
    ...
    config = {
        ...
        'save_dir': './regular_new_dataset_results_just_decoder',  
    ...
    # Model checkpoint path. Note for me: i have changed the path to the current one from ff to just decoder
    checkpoint_path = '/home/akram/Downloads/ssl_v9/ssl_v10_fc/ssl_v10_conv/ssl_v10_unet/ssl_v11_with_new_model/ssl_12_with_lora/UNETDecoder/ssl_15/ssl_16/ssl_17/ssl_18/mae_finetuning_results/checkpoint_epoch_144.pt'
